...
**Идентификация аккаунта — по логину ITMO ID** (изменено 2026-06-15; ранее
ключом был `isu_number`, ещё раньше `telegram_id`). Экрана ввода ИСУ больше
нет — вход сразу через **ITMO ID (логин + пароль)**. Номер ИСУ не вводится
вручную, а **генерируется случайно один раз при создании аккаунта и
сохраняется** как атрибут профиля (не ключ). Telegram `initData` —
только проверка безопасности (вход лишь из настоящего Telegram); «кто ты»
определяет логин ITMO ID. Пароль косметический (мок: любой непустой). Тот же
логин → тот же аккаунт. Выпуск Supabase-сессии — через **Edge Function +
Admin API**:

1. Пользователь вводит логин+пароль ITMO ID (единственный экран входа).
   Frontend вызывает мок `fetchItmoIdProfile(login, password)` (ФИО/курс/
   возраст/аватар, детерминирован по логину), получает `initDataRaw` из
   Telegram SDK и POST-ит `{ initDataRaw, login }` в Edge Function
   **`telegram-auth`** (`verify_jwt = false`).
2. Edge Function проверяет HMAC-SHA256 подпись `initData` секретом из
   `TELEGRAM_BOT_TOKEN` (алгоритм Telegram: `secret =
   HMAC_SHA256("WebAppData", bot_token)`), `auth_date` и непустой логин.
   Невалидно → 401/400.
3. По **логину** находит/создаёт пользователя в `auth.users` через Admin
   API. Учётка детерминированная: email/пароль выводятся из логина через
   HMAC(серверный секрет `AUTH_USER_SECRET`, `login`) — известны только
   серверу (логин в БД не хранится, он закодирован в email auth-юзера).
4. Гарантирует строку в `public.users` (`id` = id auth-пользователя). Если
   строка новая — **генерирует случайный уникальный `isu_number`** (6 цифр)
   и сохраняет; если уже была — `isu_number` сохраняется прежний.
   `telegram_id` = из initData как метаданные последнего входа (**не ключ**).
5. Серверно выполняет `signInWithPassword` и возвращает клиенту
   `{ access_token, refresh_token, is_new_user }`. Клиент вызывает
   `supabase.auth.setSession(...)`, затем `updateProfile` записывает
   профиль ITMO ID (`full_name`/`course`/`age`/`avatar_url`,
   `itmo_id_linked=true`). Пароль клиенту не передаётся.

**Смена аккаунта:** «Выйти» = `signOut` (сессия очищается) → экран входа
ITMO ID. Другой логин = другой аккаунт (свои поездки/роль/админка, свой
сгенерированный ИСУ); тот же логин → снова тот аккаунт. Данные при выходе
**не стираются**. `users.telegram_id` не уникальный и nullable (миграция
`..._isu_account.sql`); `users.isu_number` — уникальный сгенерированный
атрибут, ключ аккаунта — логин ITMO ID (через email auth-юзера).
...
#### `vehicle_public_info` — публичная информация об авто (2026-06-12)

Таблица `vehicles` по RLS видна только владельцу/админу, но пассажиру в
карточке поездки нужны цвет/марка/модель/госномер машины (чтобы узнать
авто при посадке). По образцу `user_public_profiles` добавлена вью
(миграция `20260612130000_vehicle_public_info.sql`):
...
Наружу не отдаются `seats_count`/`photo_url`/таймстемпы. Грант только
`authenticated` (в отличие от профилей — анону госномера не нужны).
Связь с поездкой — через `trips.driver_id` (по MVP у водителя одно авто,
берётся первое по `created_at`); `trips.vehicle_id` по-прежнему не
заполняется (отложено).
...
| `searchTrips` | `(filters: TripSearchFilters, excludeDriverId?: string) => Promise<TripWithRoute[]>` | Активные поездки с фильтрами по точкам/дате/времени (ТЗ 5.7). `excludeDriverId` исключает поездки этого водителя (пассажир не видит свои же поездки в поиске). Возвращает с развёрнутыми `origin`/`destination`/`driver`/`vehicle` (вью `vehicle_public_info`, по `driver_id`). |
...
| `getMyPassengerTrips` | `(passengerId: string) => Promise<TripRequestWithTrip[]>` | Заявки пассажира с развёрнутой поездкой, профилем водителя и его авто (`vehicle`), для «Мои поездки» (вид пассажира). |
...
| 2026-06-14 | Дизайн под ITMO: тёмная тема (авто по теме устройства/Telegram + ручной тоггл), редизайн экрана ITMO ID, единый стиль (радиус, filled-поля, `Card`/`ScreenHeader`), лого с сине-красным градиентом | Запрос пользователя на финальный дизайн |
| 2026-06-14 | **Идентификация аккаунта переведена с `telegram_id` на `isu_number`** (5.1): аккаунт = номер ИСУ (без пароля), Telegram — только для безопасности; смена ИСУ = смена аккаунта. Миграция `20260614120000_isu_account.sql` (telegram_id nullable/не уникальный), `telegram-auth` перевыпускает сессию по ИСУ | Решение пользователя |
| 2026-06-15 | **Идентификация аккаунта переведена с `isu_number` на логин ITMO ID** (5.1): экран ввода ИСУ удалён, вход сразу через ITMO ID (логин+пароль); ИСУ генерируется случайно один раз при создании аккаунта и сохраняется как атрибут. `telegram-auth` принимает `{ initDataRaw, login }`, ключит auth-юзера по логину, генерирует `isu_number`. Frontend: `AuthBackend.loginWithItmoId`, состояние `needs-login`, удалён `RegistrationScreen` | Решение пользователя |
| 2026-06-15 | **Статус «В пути» (`in_progress`) в `trip_status`** (5.2.2): водитель управляет статусом поездки (`active`→`in_progress`→`completed`, отмена), статус виден всем. Миграция `20260615120000_trip_in_progress.sql` (`ALTER TYPE … ADD VALUE`), сервис `updateTripStatus`/`canTransitionTrip` (5.5), кнопки в `MyTripsScreen` (вид водителя) + бейдж статуса поездки в карточке пассажира | Решение пользователя (статусы предложены ассистентом) |
