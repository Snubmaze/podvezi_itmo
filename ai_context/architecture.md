...
**Идентификация аккаунта — по номеру ИСУ** (изменено после деплоя; ранее
ключом был `telegram_id`). Telegram `initData` используется только для
проверки безопасности (вход возможен лишь из настоящего Telegram), но «кто
ты» определяет ИСУ. Пароля нет: ввёл ИСУ → попал в этот аккаунт (для демо;
в реале ИСУ подтверждался бы настоящим ITMO ID). Выпуск Supabase-сессии —
через **Edge Function + Admin API**:

1. Пользователь вводит номер ИСУ (экран входа). Frontend получает
   `initDataRaw` из Telegram SDK и POST-ит `{ initDataRaw, isu }` в Edge
...
2. Edge Function проверяет HMAC-SHA256 подпись `initData` секретом из
   ...
   HMAC_SHA256("WebAppData", bot_token)`), `auth_date` и формат ИСУ.
   Невалидно → 401/400.
3. По **ИСУ** находит/создаёт пользователя в `auth.users` через Admin API.
   Учётка детерминированная: email `isu<isu>@itmo.podvezi.local`, пароль =
   HMAC(серверный секрет `AUTH_USER_SECRET`, `isu`) — известен только
   серверу.
4. Гарантирует строку в `public.users` (`id` = id auth-пользователя,
   `isu_number` = ИСУ; `telegram_id` = из initData как метаданные
   последнего входа, **не ключ**). Профиль (`full_name`/`course`/`age`/
   `avatar_url`) заполняется на шаге мок-входа ITMO ID.
...
**Смена аккаунта:** «Выйти» = `signOut` (сессия очищается) → экран ввода
ИСУ. Другой ИСУ = другой аккаунт (свои поездки/роль/админка); старый ИСУ →
снова тот аккаунт. Данные при выходе **не стираются**. `users.telegram_id`
больше не уникальный и nullable (миграция `..._isu_account.sql`); ключ
аккаунта — `users.isu_number` (unique).
