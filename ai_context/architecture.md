...
1. Загрузка фото ВУ и СТС в `driver-documents` по пути
   `<userId>/<license|sts>/<ts>.<ext>` (RLS: первый сегмент = `auth.uid()`).
2. `insert vehicles` (make, model, plate_number, color, seats_count).
3. `insert driver_documents` ×2 (`license`/`sts`, `status='pending'`,
   `file_path`).
