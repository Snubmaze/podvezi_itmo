...
## Дев-помощники (мок-режим браузера)

- `services/auth.ts#devSetMockDriverStatus(status)` — DEV-only: подмена
  `driver_verification_status` мок-пользователя в localStorage.
- `ProfileScreen`: кнопка **«Выйти»** (signOut) и DEV-кнопка **«Стать
  водителем / Снять статус водителя»** (для проверки водительского флоу
  локально без Telegram/админа).

## Шаг 7. Регистрация и верификация водителя — выполнено

Дата: 2026-06-11. Без новых миграций (используются существующие
`vehicles`/`driver_documents`/`moderation_requests` + bucket
`driver-documents`). Контракт — `architecture.md` 5.6.

### Что сделано (файлы)

- `src/types/driver.ts` — `DriverApplicationInput`, `DriverApplicationSummary`.
- `src/services/driver.ts` — `submitDriverVerification(userId, input)`
  (загрузка ВУ/СТС в Storage → `vehicles` → `driver_documents` ×2 →
  `moderation_requests` → `users.driver_verification_status='pending'`;
  **мок-режим**: только переход в `pending`); `getDriverApplication(userId)`
  (последняя заявка — статус/причина; в мок-режиме `null`).
- `src/pages/DriverRegistrationScreen.tsx` (6.4) — форма: марка/модель/
  госномер/цвет/число мест + загрузка фото ВУ и СТС, валидация, отправка.
- `src/app/auth-context.ts` + `AuthProvider.tsx` — действие `reloadUser()`
  (перечитать профиль без флага loading после подачи заявки).
- `DriverVerificationModal` — кнопка «Подать заявку/снова» ведёт на экран
  регистрации (`onApply`), убрана заглушка шага 4.
- `HomeScreen` (+ `onVerifyDriver`), `AuthenticatedApp` (View
  `'driver-verify'`).
- `ProfileScreen` — причина отклонения (6.9) при статусе `rejected`
  (через `getDriverApplication`).
- `npm run build` и `npm run lint` — без ошибок.

### Поведение по статусам

- `none`/`rejected` → тап «Водитель» → модалка → «Подать заявку» → форма
  → `pending`.
- `pending` → модалка «Заявка на проверке».
- `approved` (ставит админ, шаг 8; в деве — dev-кнопка) → вкладка
  «Водитель» разблокирована, доступно создание поездок.

### Не сделано / отложено

- Установка `approved`/`rejected` и причина отклонения — **шаг 8**
  (админ-панель). Сейчас в деве апрув через dev-кнопку профиля.
- `vehicles.photo_url`, связь `trips.vehicle_id` с авто — возможное
  улучшение (поездки создаются с `vehicle_id = null`).
- Реальная загрузка в Storage и записи работают только в Telegram-режиме
  (в браузере мок — статус имитируется).

## Шаг 8. Админ-панель модерации — выполнено

Дата: 2026-06-11. Без новых миграций (существующие
`moderation_requests`/`driver_documents`/`vehicles`/`users` + RLS
`is_admin()`). Контракт — `architecture.md` 5.7. Решения пользователя:
блокировку пользователей **отложить**; тест админки локально — **dev-роль
+ мок-заявки**.

### Что сделано (файлы)

- `src/types/moderation.ts` — `DriverApplicationReview`, `ReviewVehicle`,
  `ReviewDocument`.
- `src/services/moderation.ts` — `getPendingDriverApplications` (заявки +
  профиль/авто/документы, signed-URL на фото), `approveDriverApplication`,
  `rejectDriverApplication(... reason)`; мок-ветка с демо-заявками.
- `src/hooks/usePendingApplications.ts` — загрузка заявок + `refetch`.
- `src/pages/AdminModerationScreen.tsx` — список заявок «На проверке»,
  данные заявителя/авто, превью документов (открытие по signed-URL),
  кнопки «Подтвердить»/«Отклонить» (модалка с причиной).
- `src/services/auth.ts` — dev-хелпер `devSetMockRole(role)`.
- `ProfileScreen` — кнопка «Модерация заявок» (при `role='admin'`) +
  DEV-кнопка «Стать админом»; `AuthenticatedApp` — View `'admin'`.
- `npm run build` и `npm run lint` — без ошибок.

### Доступ и проверка

- UI админки — только при `role='admin'`. Жёсткое ограничение — RLS
  (`is_admin()` на `moderation_requests`/`driver_documents`/чужих
  `users`/`vehicles`) + триггер `guard_users_update` (проверено на шаге 4:
  self-approve/self-admin → 400). Назначение реального админа —
  `update users set role='admin'` через SQL Editor.
- Решения админа отражаются у заявителя: профиль (6.9) показывает причину
  отклонения, статус → разблокировка/блокировка вкладки «Водитель».

### Не сделано / отложено

- **Блокировка пользователей** (`users.is_blocked` + enforcement) —
  отдельный подшаг (нужна миграция).
- Реальные данные/signed-URL работают только в Telegram-режиме (в браузере
  — демо-заявки из памяти).

## Фикс: просмотр фото документов в модерации — открывается в апке, а не в браузере

Дата: 2026-06-12.

- Новый компонент `src/components/ImageLightbox.tsx` — полноэкранный
  просмотр фото внутри Mini App (оверлей `bg-black/90`, `object-contain`,
  кнопка закрытия, Esc/клик-вне). Добавлен в `design_system.md`.
- `src/pages/AdminModerationScreen.tsx` (`DocThumb`/`ApplicationCard`):
  превью документа теперь открывает `ImageLightbox` (по клику на фото),
  вместо `<a target="_blank">`, открывавшего фото в системном браузере.
- `npm run build` и `npm run lint` — без ошибок.

## Фикс: валидация формата фото документов (ВУ/СТС)

Дата: 2026-06-12.

- `src/pages/DriverRegistrationScreen.tsx` — `FileField`: допустимые
  форматы фото — JPG/JPEG/PNG/HEIC/HEIF/WEBP (`ALLOWED_IMAGE_TYPES` +
  fallback по расширению файла для HEIC/HEIF, у которых браузер часто не
  отдаёт MIME-тип). При выборе файла другого формата — ошибка под полем,
  файл не принимается. `accept` у `<input type="file">` обновлён под этот
  список.
- `npm run build` и `npm run lint` — без ошибок.

## Фикс: поля даты/времени — в одну строку без наложения

Дата: 2026-06-12 (заменяет предыдущий фикс «поля в столбик на мобильных»).

- `src/components/TripSearchFilterBar.tsx` (фильтры поиска) и
  `src/pages/TripCreateScreen.tsx` (дата/время отправления) — поля
  «Дата»/«Время» снова в одну строку (`grid-cols-2`) на всех ширинах.
  Причина прежнего наложения: у grid-ячеек `min-width: auto`, из-за чего
  колонки не ужимались меньше внутренней ширины нативных
  `<input type="date">`/`<input type="time">`. Исправлено добавлением
  `min-w-0` на ячейки (сам `Input` уже имеет `w-full min-w-0`).
- Дополнение (по скриншоту с iOS): `min-w-0` не помогло в iOS WebKit —
  UA-стили нативных date/time-инпутов (shadow DOM) держат минимальную
  ширину по контенту. В `src/index.css` (`@layer base`) добавлен сброс
  `appearance: none` для `input[type=date]`/`input[type=time]` +
  выравнивание значения влево и `min-height` через
  `::-webkit-date-and-time-value`. Отражено в `design_system.md`.
- `npm run build` и `npm run lint` — без ошибок.

## Фича: кликабельное имя водителя в карточке поездки → профиль водителя

Дата: 2026-06-12.

- `src/components/TripCard.tsx` — новый опциональный проп
  `onDriverClick?: (driverId: string) => void`: если задан, ФИО водителя
  рендерится кнопкой (подчёркивание, focus-ring) и открывает профиль.
- `src/app/AuthenticatedApp.tsx` — новый view `driver-profile`:
  открывает существующий `CoTravelerProfileScreen`, помнит экран
  возврата (`home` или `my-trips`) для кнопки «Назад».
- Подключено в двух местах: поиск поездок на главном экране
  (`HomeScreen` → `TripSearchResultCard`) и «Мои поездки», вид пассажира
  (`MyTripsScreen` → `PassengerRequestCard`).
- `src/pages/CoTravelerProfileScreen.tsx` — фолбэк: если RPC
  `get_co_traveler_contact` вернул `null` (пассажир ещё не подтверждён),
  показывается публичный профиль (имя, аватар, курс) без контактов +
  подсказка «контакты станут видны после подтверждения заявки».
- `src/services/trips.ts` — новая функция `getPublicProfile(userId)`
  (одиночный профиль из вью `user_public_profiles`).
- `npm run build` и `npm run lint` — без ошибок.

## Фича: информация об авто в карточке поездки

Дата: 2026-06-12.

- Новая SQL-вью `vehicle_public_info` (миграция
  `supabase/migrations/20260612130000_vehicle_public_info.sql`, по образцу
  `user_public_profiles`): `id`, `driver_id`, `make`, `model`, `color`,
  `plate_number` из `vehicles` (RLS таблицы скрывает её от пассажиров),
  grant только `authenticated`. **Миграцию нужно применить вручную через
  SQL Editor.**
- `src/types/db.ts` — тип `VehiclePublicInfo`; `src/types/trips.ts` —
  `TripWithRoute.vehicle: VehiclePublicInfo | null`.
- `src/services/trips.ts` — хелпер `fetchDriverVehicles(driverIds)`
  (одно авто на водителя, MVP); подключён в `searchTrips` и
  `getMyPassengerTrips` (параллельно с профилями). В `getMyDriverTrips`
  `vehicle: null` (водитель своё авто знает).
- `src/components/TripCard.tsx` — при `showDriver` и наличии `vehicle`
  строка: иконка `Car`, «цвет марка модель», госномер плашкой
  (`bg-muted`, `font-mono`, `uppercase`). Отражено в `design_system.md`.
- `trips.vehicle_id` по-прежнему не заполняется — связь через
  `driver_id` (см. `architecture.md` 5.2).
- `npm run build` и `npm run lint` — без ошибок.
...
- Прод-сервер `186.246.14.187` (nginx, Let's Encrypt). Приложение
  собрано (`npm run build`) и раздаётся по `https://ap2-409371.lab-itmo.ru/`
  — реальный HTTPS-хост для тестирования Telegram Mini App (initData,
  шаги 3-4).
- Добавлен `.github/workflows/deploy.yml`: при пуше в `main` —
  `npm ci && npm run build` (с `VITE_*` из GitHub Secrets) и rsync `dist/`
  на сервер по SSH от пользователя `deploy` (создан на сервере, владеет
  `/var/www/ap2-409371.lab-itmo.ru`, отдельный ключ только для CI).
- См. `architecture.md` §6 для деталей и списка нужных secrets.

## Доработки после реальных тестов (2026-06-13)

- **Выход = отвязка/ре-онбординг** (решение пользователя). Кнопка «Выйти из
  аккаунта» в профиле (с подтверждением) сбрасывает привязку (`isu_number`,
  `itmo_id_linked`, ФИО/курс/возраст/аватар из ITMO) и возвращает к
  регистрации; статус верификации водителя и роль сохраняются. Действие
  провайдера `logout()` (бывш. `signOut`), работает в мок- и Supabase-
  режимах. Причина: в TG авторизация автоматическая по `initData`, обычный
  logout бессмыслен.
- **Лейбл роли в профиле** — `profileRoleBadge(user)` (`src/lib/verification.ts`):
  `Админ` / `Водитель` (approved) / `В процессе верификации` (pending) /
  `Пассажир` (иначе), вместо статуса верификации под именем. Причина
  отклонения (rejected) по-прежнему отдельной строкой.
- **Поиск поездок пассажиром** — `searchTrips` дополнительно исключает
  поездки, на которые пользователь уже подал заявку (`pending`/`accepted`).
- **Валидация файлов заявки водителя** — только изображения (JPG/PNG/HEIC/
  HEIF/WEBP), проверка по MIME и расширению — уже было реализовано.

## Привязка аккаунта к ИСУ (2026-06-14)

Аккаунт теперь определяется **номером ИСУ**, а не `telegram_id` (см.
`architecture.md` 5.1). Смена ИСУ = смена аккаунта (свои поездки/роль/
админка); «Выйти» = `logout` (сессия очищается, данные сохраняются) →
экран ввода ИСУ. Пароля нет (ITMO-логин косметический).

- **Миграция** `supabase/migrations/20260614120000_isu_account.sql` —
  `users.telegram_id` больше не UNIQUE и nullable (ключ — `isu_number`).
  **Нужно применить вручную (SQL Editor).**
- **Edge Function** `telegram-auth` переписана: принимает
  `{ initDataRaw, isu }`, проверяет подпись Telegram (безопасность) и
  ключит auth-пользователя по ИСУ (email `isu<isu>@itmo.podvezi.local`).
  **Нужно перевыпустить (Dashboard → Edge Functions).**
- **Frontend**: `services/auth.ts` — интерфейс `AuthBackend`
  (`authenticate` → `User | null`, `loginWithIsu`, `updateProfile`,
  `logout`); мок хранит аккаунты по ИСУ в localStorage
  (`podvezi.mock.accounts` + `podvezi.mock.currentIsu`). `AuthProvider`/
  `auth-context` — состояние `needs-isu`, действия `loginWithIsu`/`logout`
  (убран `setIsuNumber`/`signOut`-unlink). `AppFlow` — `needs-isu` →
  экран ввода ИСУ. `RegistrationScreen` → вход по ИСУ. `ProfileScreen` —
  текст выхода обновлён.
- В браузерном моке работает сразу; для реального Telegram — применить
  миграцию и перевыпустить функцию. Старые аккаунты (по `telegram_id`)
  станут не видны (создадутся заново по ИСУ).
- `npm run build` и `npm run lint` — без ошибок.

## Вход через ITMO ID (аккаунт = логин) (2026-06-15)

Аккаунт теперь определяется **логином ITMO ID**, а не номером ИСУ (см.
`architecture.md` 5.1). Экран ввода ИСУ удалён — вход сразу через ITMO ID
(логин+пароль). Номер ИСУ генерируется случайно один раз при создании
аккаунта и сохраняется как атрибут профиля (вместе с ФИО/курсом/возрастом из
мок ITMO ID). Тот же логин → тот же аккаунт; «Выйти» = `logout` → экран
входа ITMO ID.

- **Frontend**: `services/auth.ts` — `AuthBackend.loginWithIsu` →
  `loginWithItmoId(login, profile)` (мок: аккаунты в localStorage по логину,
  ключ `podvezi.mock.currentLogin`; новый аккаунт получает случайный ИСУ
  через `generateIsuNumber()`). `auth-context`/`AuthProvider` — состояние
  `needs-isu` → `needs-login`, действие `loginWithItmoId(login, password)`
  (внутри `fetchItmoIdProfile` + backend), удалён `linkItmoId`. `AppFlow` —
  `needs-login` → `ItmoIdLoginScreen`. `ItmoIdLoginScreen` — единственный
  экран входа. **Удалён** `pages/RegistrationScreen.tsx` (экран ввода ИСУ).
  `ProfileScreen` — текст выхода обновлён.
- **Edge Function** `telegram-auth` переписана: принимает
  `{ initDataRaw, login }`, проверяет подпись Telegram (безопасность) и
  ключит auth-пользователя по логину (email/пароль из
  `HMAC(AUTH_USER_SECRET, login)`); при создании строки `public.users`
  генерирует случайный уникальный `isu_number`. **Нужно перевыпустить**
  (Dashboard → Edge Functions). Профиль ITMO ID клиент дописывает через
  `updateProfile` после `setSession`.
- **Миграция** `20260614120000_isu_account.sql` (telegram_id nullable/не
  уникальный) по-прежнему обязательна (тот же Telegram может относиться к
  нескольким аккаунтам). **Применить вручную (SQL Editor)**, если ещё нет.
- В браузерном моке работает сразу; для реального Telegram — применить
  миграцию и перевыпустить функцию.
- `npm run build` и `npm run lint` — без ошибок.

## Смена статуса поездки водителем (2026-06-15)

Водитель может менять статус поездки вплоть до завершённой; статус виден всем
(в т.ч. пассажирам в «Мои поездки»). Набор статусов (предложен ассистентом,
`architecture.md` 5.2.2): **Активна** (`active`) → **В пути** (`in_progress`)
→ **Завершена** (`completed`); **Отменена** (`cancelled`) только из `active`
(отменить можно лишь до старта поездки). Завершённая/отменённая — финальные.

- **Миграция** `supabase/migrations/20260615120000_trip_in_progress.sql` —
  `alter type public.trip_status add value 'in_progress' after 'active'`.
  **Применить вручную (SQL Editor).**
- `src/types/db.ts` — `TripStatus` += `'in_progress'`.
- `src/lib/tripStatus.ts` — бейдж «В пути» (`warning`).
- `src/services/trips.ts` — `updateTripStatus(tripId, status)` (UPDATE под RLS
  водителя/админа) и `canTransitionTrip(from, to)` + карта
  `TRIP_STATUS_TRANSITIONS` (контракт — `architecture.md` 5.5).
- `src/pages/MyTripsScreen.tsx` — у водителя в карточке поездки кнопки
  переходов («Начать поездку»/«Завершить»/«Отменить поездку», по допустимым
  переходам); у пассажира — бейдж статуса поездки, когда он ≠ `active`.
  Realtime-подписка на `trips` обновляет обе стороны автоматически.
- `searchTrips` по-прежнему ищет только `active` (присоединиться можно лишь к
  активной); водитель видит свои поездки всех статусов.
- `npm run build` и `npm run lint` — без ошибок.

## Следующий шаг

По `plan.md` далее — **шаг 9 (UI/UX полировка: dark/light, Telegram
themes)**. Также в backlog: «поездки без водителя» (release 2) и блокировка
пользователей. Финальный выбор следующего шага — за пользователем.
