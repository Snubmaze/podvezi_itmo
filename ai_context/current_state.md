...
## Фикс: поля даты/времени пересекались на мобильных экранах

Дата: 2026-06-12.

- `src/components/TripSearchFilterBar.tsx` (главный экран пассажира,
  фильтры поиска) и `src/pages/TripCreateScreen.tsx` (создание поездки,
  дата/время отправления) — сетка `grid-cols-2` для полей «Дата»/«Время»
  заменена на `grid-cols-1 sm:grid-cols-2`: на узких мобильных экранах
  нативные `<input type="date">`/`<input type="time">` не помещались в
  половину ширины и накладывались друг на друга, теперь они в столбик на
  мобильных и в 2 колонки от `sm` (≥640px).
- `npm run build` и `npm run lint` — без ошибок.
